...
(function () {
	...
	kazoo.attachMedia = function(session) {
		console.log('Attaching media to session', session);
		...
		kazoo.videoRemote.srcObject = remoteStream;
		*/
		...
		if (remoteStream) {
			console.debug('Already have a remote stream');

			handleRemoteStream(remoteStream);
		}

		console.log('peerConnection', peerconnection);
		kazoo.peerConnection = peerconnection;
		peerconnection.addEventListener('addstream', function(event) {
			console.debug('Added stream to connection');
		...
		function handleRemoteStream(stream) {
			console.debug('Handle remote stream', stream);
			...
			stream.addEventListener('addtrack', function(event) {
				...
				console.debug('Added track to remote stream', track);

				// Refresh remote video
				remoteVideo.srcObject = stream;

				track.addEventListener('ended', function() {
					console.debug('Remote track ended', track);
				});
			});
			...
			stream.addEventListener('removetrack', function() {
				...
				console.debug('Removed track from remote stream');
		...
		/*
		// Old
		var remoteEl = document.getElementById('remoteMedia');
		var localEl = document.getElementById('localMedia');

		console.log('els', remoteEl, localEl);

		var pc = session.sessionDescriptionHandler.peerConnection;
		console.info('peerConnection', pc);

		// Gets remote tracks
		var receivers = pc.getReceivers();
		console.info('receivers', receivers);
		if ( receivers.length > 0 ) {
			var remoteStream = new MediaStream();
			receivers.forEach(function(receiver) {
				console.log('recv track', receiver.track);
				remoteStream.addTrack(receiver.track);
			});
			remoteEl.srcObject = remoteStream;
			remoteEl.play();
		} else {
			console.log('no receivers to track');
		}

		// Gets local tracks
		var senders = pc.getSenders();
		console.info('senders', senders);
		if ( senders.length > 0 ) {
			var localStream = new MediaStream();
			senders.forEach(function(sender) {
				console.log('send track', receiver.track);
				localStream.addTrack(sender.track);
			});
			localEl.srcObject = localStream;
			localEl.play();
		} else {
			console.log('no senders to track');
		}
		*/
	};
		...
		if(kazoo.version === 'rtmp') {
			...
			kazoo.sipjs.userAgent = new SIP.UA({
				...
				transportOptions: {
					...
					reconnectionTimeout: (!isNaN(params.reconnectDelay) && params.reconnectDelay < 5) ? 5 : params.reconnectDelay
				},
			...
			kazoo.sipjs.userAgent.on('invite', function(session) {
				...
				if(typeof kazoo.sipjs.session === 'undefined') {
					...
					var call = {
						accept: function() {
							//kazoo.attachMedia(kazoo.sipjs.session);
			...
			kazoo.sipjs.bindSessionEvents = function(session) {
				...
				session.on('accepted', function(arg) {
					...
					var remoteVideo = kazoo.videoRemote;
					var streams = session.sessionDescriptionHandler.peerConnection.getRemoteStreams();
				...
				session.on('referRequested', function(arg) {
					console.info('referRequested', arg);
